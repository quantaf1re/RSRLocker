...
def test__initial_locker(a, ics, lockerSwap, lockerWeights):
    ...
    for tx in [lockerSwap, lockerWeights]:
        ...
        assert tx.events["Locked"][0]["amount"] == INITIAL_RSR_AMOUNT_TO_LOCK
    ...
    for locker in [ics.lockerSwap, ics.lockerWeights]:
        ...
        assert proposal.state() == STATE_TO_NUM[CREATED]

        # Out of my scope to test that the proposals were made correctly, only
        # need to test that they exist
        assert ics.manager.trustedProposals(proposal_id) != ZERO_ADDRESS
        ...
        assert locker.lockLength() == INITIAL_PROPOSAL_LOCK_TIME
        assert ics.rsr.balanceOf(locker.address) == INITIAL_RSR_AMOUNT_TO_LOCK
